...
int main() {
    ...
    for (int y = 0; y < iterations ; y++) {
        ...
        beta = (iterations - 1 - y) / (iterations - 1) / Tmax + y / (iterations - 1); //beta(y)
        int check1 = 0;
        int check2 = 0;
        for (int w = 0; w < Npopulation; w++) {
            Individual parent1 = Selector();
            Individual parent2 = Selector();
            Individual appo1 = parent1;
            Individual appo2 = parent2;
            double r = rnd.Rannyu(); //random number for the method
            double a = rnd.Rannyu(); //random number for the annealing
            if (r <= prob) {
                PairMutation(appo1);
            } else if (r <= prob*2) {
                ShiftMutation(appo1);
            } else if (r <= prob*3) {
                PermutationMutation(appo1);
            } else if (r <= prob*4) {
                InversionMutation(appo1);
            } else {
                CrossingOver(appo1, appo2);
            }
            check1 = CheckFunction(appo1);
            check2 = CheckFunction(appo2);
            if (check1 == 1 || check2 == 1) {
                cout << "Error in the creation of an individual" << endl;
            }
            appo1.SetL(L(appo1));
            appo2.SetL(L(appo2));
            if (a < exp(-beta * (appo1.GetL() - parent1.GetL()))) //simulated annealing
                nextGen[w].SetCities(appo1.GetCities());
            else
                nextGen[w].SetCities(parent1.GetCities());
            if (r > prob*4 && w < Npopulation - 1) {
                w++;
                if (a < exp(-beta * (appo2.GetL() - parent2.GetL())))
                    nextGen[w].SetCities(appo2.GetCities());
                else
