...
{ 
  ...
  for (int c = 0; c < 31; c++) { 
      if (c == 0) {
          ...
          WriteData.open("mag_gibbs.dat", ios::app);
          //WriteData << temp << '\t' << glob_av[iu] / (double)nblk << '\t' << err_u << '\t' << glob_av[ic] / (double)nblk << '\t' << err_c << '\t' << glob_av[ix] / (double)nblk << '\t' << err_x << endl;
          WriteData << temp << '\t' << glob_av[im] / (double)nblk << '\t' << err_m << endl;
          WriteData.close();
      }
      else {
          temp -= 0.05;
...
//Read input informations
  ...
  ReadInput >> start; // if=0 random config.
...
//Print initial values for the potential energy and virial
  ...
  cout << "Initial heat capacity = " << pow(beta, 2) * (walker[ic] - pow(walker[iu], 2)) / (double)nspin << endl;
  cout << "Initial magnetization = " << walker[im] / (double)nspin << endl;
  cout << "Initial susceptibility = " << beta * (walker[ix] - pow(walker[im], 2)) / (double)nspin << endl;
}
...
void Move(int metro)
{
  ...
  //Select randomly a particle (for C++ syntax, 0 <= o <= nspin-1)
    ...
    {
        energy_up = -2 * beta * (J * (s[Pbc(o - 1)] + s[Pbc(o + 1)]) + h);
        p = 1 / (1 + exp(energy_up));
        if (rnd.Rannyu() <= p) {
            s[o] = 1;
        }
